...
class SystemControl:
    """Class endpoint for local Python tool usage."""
    def __init__(self):
        pass
    ...
    def open_app(self, name: str):
        try:
            ...
            subprocess.Popen(["powershell", "-Command", f"Start-Process '{name}'"], creationflags=creationflags)
            return True
        except: return False
    ...
    def take_screenshot(self):
        try:
            ...
            return "Saved to screenshot.jpg"
...
def _get_redirect_uri() -> str:
    ...
    # Fallback to config public URL or localhost
    base = config.BLITZ_PUBLIC_URL if config.BLITZ_PUBLIC_URL else "http://localhost:8000"
    return f"{base.rstrip('/')}/api/sys/spotify/callback"
